...
contract CampaignManager is Ownable {
    ...
    function setFactory(address _factory) external onlyOwner {
        require(_factory != address(0), "CampaignManager: Factory address cannot be zero");
    ...
    function addCampaignOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "CampaignManager: Owner address cannot be zero");
        require(!campaignOwners[_owner], "CampaignManager: Already a campaign owner");
    ...
    function removeCampaignOwner(address _owner) external onlyOwner {
        require(campaignOwners[_owner], "CampaignManager: Not a campaign owner");
    ...
    function createCampaign(
        ...
        require(address(factory) != address(0), "CampaignManager: Factory not set");
        require(campaignOwners[msg.sender], "CampaignManager: Caller is not a campaign owner");
        
        return factory.createCampaign(_name, _tokenAddress, _taxAddress, _taxPercentageBps);
    }
